    ...
    DiscordSocketClient client) : IGuildChannelsRepository
{
    ...
    public async Task SyncDbVoiceChannelsWithGuildAsync(ulong guildId, CancellationToken token = default)
    {
        ...
        await using var transaction = await mlkAdminDbContext.Database.BeginTransactionAsync(cancellationToken: token);
        ...
        try
        {
            ...
            if (toUpdate.Any())
            {
                ...
                {
                    var dbChannel = await mlkAdminDbContext.VChannels.FindAsync(vChannel.Id, token);
            ...
            if (toAdd.Any())
            {
                ...
                {
                    mlkAdminDbContext.VChannels.Add(new GuildVoiceChannel()
                    {
            ...
            await mlkAdminDbContext.SaveChangesAsync(cancellationToken: token);
